...
import { cleanup, fireEvent, render, wait, screen } from '@testing-library/react';

const loginLocations = {
  data: {
    entry: [{ resource: { id: '111', name: 'Earth' } }, { resource: { id: '222', name: 'Mars' } }],
  },
};
...
describe(`<LocationPicker />`, () => {
  ...
  it('shows error message when no matching locations can be found', async () => {
    ...
    await wait(() => {
      expect(wrapper.getByText('Sorry, no matching location was found')).not.toBeNull();
    });
    expect(submitButton).toHaveAttribute('disabled');
  });

  it('should get user Default location on render and auto select the location', async () => {
    expect(window.localStorage.getItem('userDefaultLoginLocationKeyDemo')).toEqual('111');
    const locationRadioButton: HTMLElement = await wrapper.getByRole('radio', {
      name: /Earth/,
    });
    expect(locationRadioButton).toHaveProperty('checked', true);
  ...
  it('should set user Default location when location is changed', async () => {
    const locationRadioButton: HTMLElement = await wrapper.findByLabelText(/Earth/);
    fireEvent.click(locationRadioButton);
    expect(window.localStorage.setItem).toHaveBeenCalled();
    expect(window.localStorage.setItem).toHaveBeenCalledWith('userDefaultLoginLocationKey', '111');
  });
